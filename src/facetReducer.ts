...
export class FacetReducer {
  ...
  public static async reduceFacets(
    ...
    const colorDistances: number[][] =
      ColorReducer.buildColorDistanceMatrix(colorsByIndex);
  ...
  private static deleteFacet(
    ...
    if (facetToRemove.neighbourFacets!.length > 0) {
      ...
      // a facet is rebuilt
      for (
        let j: number = facetToRemove.bbox.minY;
        j <= facetToRemove.bbox.maxY;
        j++
      ) {
        for (
          let i: number = facetToRemove.bbox.minX;
  ...
  private static rebuildChangedNeighbourFacets(
    ...
    for (const neighbourIdx of facetToRemove.neighbourFacets!) {
      ...
      if (neighbour != null) {
        for (
          let y: number = neighbour.bbox.minY;
          y <= neighbour.bbox.maxY;
          y++
        ) {
          for (
            let x: number = neighbour.bbox.minX;
            x <= neighbour.bbox.maxX;
            x++
          ) {

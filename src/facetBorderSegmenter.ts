...
export class FacetBoundarySegment {
  constructor(
    public originalSegment: PathSegment,
...
export class FacetBorderSegmenter {
   ...
   */
  public static async buildFacetBorderSegments(
    ...
    const segmentsPerFacet: Array<Array<PathSegment | null>> =
  ...
  private static prepareSegmentsPerFacet(facetResult: FacetResult) {
    const segmentsPerFacet: Array<Array<PathSegment | null>> = new Array(
      facetResult.facets.length
    );
            ...
            if (oldNeighbour !== curNeighbour) {
              ...
              if (oldNeighbour !== -1) {
                ...
                if (
                  ...
                  //   +---v---+
                  if (
                    (prevBorderPoint.orientation === OrientationEnum.Top &&
                      curBorderPoint.orientation === OrientationEnum.Left) ||
                    (prevBorderPoint.orientation === OrientationEnum.Left &&
                      curBorderPoint.orientation === OrientationEnum.Top)
                  ) {
                    ...
                    (prevBorderPoint.orientation === OrientationEnum.Top &&
                      curBorderPoint.orientation === OrientationEnum.Right) ||
                    (prevBorderPoint.orientation === OrientationEnum.Right &&
                      curBorderPoint.orientation === OrientationEnum.Top)
                  ) {
                    const diagNeighbour = facetResult.facetMap.get(
                      curBorderPoint.x + 1,
                    ...
                    (prevBorderPoint.orientation === OrientationEnum.Bottom &&
                      curBorderPoint.orientation === OrientationEnum.Left) ||
                    (prevBorderPoint.orientation === OrientationEnum.Left &&
                      curBorderPoint.orientation === OrientationEnum.Bottom)
                  ) {
                    const diagNeighbour = facetResult.facetMap.get(
                      curBorderPoint.x - 1,
                      curBorderPoint.y + 1
  ...
  private static reduceSegmentComplexity(
    facetResult: FacetResult,
    segmentsPerFacet: Array<Array<PathSegment | null>>,
   ...
   */
  private static async matchSegmentsWithNeighbours(
    facetResult: FacetResult,
    segmentsPerFacet: Array<Array<PathSegment | null>>,
    onUpdate: ((progress: number) => void) | null = null
  ) {
